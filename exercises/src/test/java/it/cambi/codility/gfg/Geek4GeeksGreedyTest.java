 ...
 *
 */
public class Geek4GeeksGreedyTest
{
    ...
    private String ordinalIndexOf(String str, String substr, int n)
    {
        ...
        /**
         * If i have found requested frequency and it is not the last occurrence and there is at least one more possible character to match
         */
    ...
    private int firstElementToOccourKTime(String input, int freqToCheck)
    {
        ...
        Integer[] array = Arrays.asList(input.split(" ")).stream().map(Integer::valueOf).toArray(Integer[]::new);
        ...
        for (int i = 0; i < array.length; i++)
        {
            ...
            Map<Integer, Integer> set1 = freqTovalue.getOrDefault(freq + 1, new HashMap<Integer, Integer>());
        ...
        try
        {
            ...
            return freqTovalue.entrySet().stream().filter(m -> m.getKey() == freqToCheck).findFirst().orElse(null)
                    .getValue().entrySet().stream().sorted(Comparator.comparingInt(Map.Entry::getValue)).findFirst()
    ...
    private String uncommonCharacters(String s1, String s2)
    {
        ...
        return set1.stream().sorted().map(e -> Character.toString((char) e.intValue())).reduce("", String::concat);
    }
    ...
    private int extractMaximum(String s)
    {
        ...
        return Arrays.asList(s1).stream().filter(c -> !c.isEmpty()).mapToInt(Integer::valueOf).max().getAsInt();
    }
