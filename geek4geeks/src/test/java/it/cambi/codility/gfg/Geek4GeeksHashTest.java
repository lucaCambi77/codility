...
class Geek4GeeksHashTest {
  ...
  public void secondMostRepeatedString() {
    assertEquals(
        "bbb", secondMostRepeatedString(new String[] {"bbb", "aaa", "ccc", "bbb", "aaa", "aaa"}));
    assertEquals(
        "for",
        secondMostRepeatedString(new String[] {"geeks", "for", "geeks", "for", "geeks", "aaa"}));
  }
  ...
  public void isIsogram() {
    ...
    for (int i = 0; i < data.toCharArray().length; i++) {
      if (set.add(data.charAt(i))) continue;
  ...
  public void subArrayExists() {
    ...
    for (int i = 0; i < arr.length; i++) {
      ...
      if (prefixSumSet.contains(sum)) System.out.println("Yes");
